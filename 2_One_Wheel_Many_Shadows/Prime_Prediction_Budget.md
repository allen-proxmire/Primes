...
Put Tier 1 to work — from $p$, walk *up the wheel-open slots* and test them in order — and measure how many you need. Over all 70,434 primes in the doubling window $[10^6, 2\times 10^6]$ (both rows regenerated by [`ninety_percent_table.py`](../4_Philosophy_Ontology/repro/ninety_percent_table.py)):

| open slots tested | 1 | 2 | 3 | 4 | 5 | 6 | **7** | **8** |
|---|---|---|---|---|---|---|---|---|
| P(next prime caught), mod-210 | 30% | 51% | 66% | 77% | 84% | 89% | **93%** | 95% |
| P(next prime caught), mod-30 | 25% | 44% | 59% | 70% | 78% | 84% | 89% | **92%** |

**[emp]** So: **the next prime is within the next ~7 (mod-210) or ~8 (mod-30) wheel-open slots about $90\%$ of the time** — a mean of 3.25 (mod-210) or 3.79 (mod-30) tests. That $90\%$ is bought *entirely* by base rate + wheel.
