...
- **[emp] The switchback law — scale-invariant angle-sign structure.** Keep only the *sign* of each prime-angle change (up/down): the run-length distribution of same-direction runs is the same in the hundreds and the millions ($\approx63\%/27\%/8\%/1.6\%$ for lengths $1/2/3/4$), even though the wobble amplitude shrinks $\sim10^4\times$ — a visible face of the scale-invariance of normalized prime gaps. Runs die faster than a coin, so the sign is predictable: reversal odds climb $63\to74\to80\to82\%$ with run length (a scale-transferable betting rule). Fine structure: $|\Delta g|\equiv0\pmod 6$ is suppressed (the wheel, à la Lemke Oliver–Soundararajan). → [Switchback Law](2_One_Wheel_Many_Shadows/Switchback_Law.md)
