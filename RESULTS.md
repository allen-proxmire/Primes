...
- **[verified/emp] Prime-triangle angle is a rescaled gap:** $\alpha_n = \arctan(p_n/p_{n+1}) \approx 45° - \frac{90}{\pi}\frac{g_n}{p_{n+1}}$ — angles rise to a $45°$ ceiling in gap-family arcs (twins on top, never reaching it). The wobble is ~99% universal (a $33\ln p/p$ envelope, a $1/3$ runs constant, the exact $-\tfrac12$ differencing recoil); the lone ~1% prime signal is a consecutive-gap anti-correlation $-0.05$ reproduced by the wheel — *consistent with, not identical to,* the Hardy–Littlewood pair-correlation (which is the separate, positive offset comb). → [Angle Wobble](2_One_Wheel_Many_Shadows/PG_Angle_Wobble.md)
- **[emp] The switchback law — scale-invariant angle-sign structure.** Keep only the *sign* of each prime-angle change (up/down): the run-length distribution of same-direction runs is the same in the hundreds and the millions ($\approx63\%/27\%/8\%/1.6\%$ for lengths $1/2/3/4$), even though the wobble amplitude shrinks $\sim10^4\times$ — a visible face of the scale-invariance of normalized prime gaps. Runs die faster than a coin, so the sign is predictable: reversal odds climb $63\to74\to80\to82\%$ with run length (a scale-transferable betting rule). Fine structure: $|\Delta g|\equiv0\pmod 6$ is suppressed (consecutive gaps avoid a shared residue mod 6, nearly 2:1) — confirmed to be *leading-order Hardy–Littlewood*: a wheel-Cramér model matches the dip depths to $0.1$ point, with a small decaying Lemke Oliver–Soundararajan tail. → [Switchback Law](2_One_Wheel_Many_Shadows/Switchback_Law.md)
- **[emp/null] Prime-gap "weather" is mostly a differencing artifact.** The eye-catching volatility clustering ($+0.31$), big-jump recoil ($82\%$), and a $44\%$-predictable windowed wobble nearly all vanish under the transform-matched null (shuffle the gaps, *then* difference — first differences of any i.i.d. sequence carry lag-1 autocorrelation exactly $-\tfrac12$). What survives is small and real: $\approx+2$ pp of genuine windowed-wobble memory (~6× the raw-gap signal), i.e. the wheel in consecutive gaps (Lemke Oliver–Soundararajan). The right null is the whole game. → [Differencing Trap](2_One_Wheel_Many_Shadows/Prime_Gap_Memory_Differencing_Trap.md)
